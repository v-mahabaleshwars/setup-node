...
> The action resolves a relative `node-version-file` path against the workspace directory (`GITHUB_WORKSPACE`), which is the repository root by default. An absolute path is used as provided instead of being appended to the workspace, which is useful inside composite actions, for example `node-version-file: ${{ github.action_path }}/.nvmrc`.
